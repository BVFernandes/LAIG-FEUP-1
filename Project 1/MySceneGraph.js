...
MySceneGraph.prototype.parseArgsPrimitives = function(values, type) {
	...
	case 'sphere':{
		if(valuesS.length != 3)
			...
			let temp = parseFloat(valuesS[0]);
			this.checkArgsPrimitives(temp,type, 0);
			vals.push(temp);

			temp = parseInt(valuesS[1]);
			this.checkArgsPrimitives(temp,type, 1);
			vals.push(temp);

			temp = parseInt(valuesS[2]);
...
MySceneGraph.prototype.checkArgsPrimitives = function(value, type, index){
	if (value == null ) {
...
MySceneGraph.prototype.checkArgsPatches = function(value, coordinate){
	if (value == null ) {
		this.onXMLMinorError("unable to parse" + coordinate + "of patch");
	}
	else if (isNaN(value))
