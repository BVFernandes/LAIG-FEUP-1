            ...
            for (var j = 0; j < descendants.length; j++) {
                if (descendants[j].nodeName == "NODEREF")
				{
					...
					if (descendants[j].nodeName == "LEAF")
					{
						...
						if(args == null)
							this.warn("Error in leaf");
						var argsVal = this.convertStringtoValues(args,type);
...
MySceneGraph.prototype.convertStringtoValues = function(values, type) {
	var vals = [];
	var valuesS = values.split(" ");
	if(type == 'rectangle'){
		if(valuesS.length != 4)
			this.onXMLError("Wrong number of arguments for leaf type: RECTANGLE");
		else{
			for(var i=0;i<4;i++)
		...
		if(valuesS.length != 3)
			this.onXMLError("Wrong number of arguments for leaf type: SPHERE");
		else{
		...
		if(valuesS.length != 9)
			this.onXMLError("Wrong number of arguments for leaf type: TRIANGLE");
		else{
			for(var i=0;i<9;i++)
				vals.push(parseFloat(valuesS[i]));;
		}
...
MySceneGraph.prototype.displayScene = function() {
	...
	var nodesDes = this.nodes["root"].leaves;
	for(var i=0;i < nodesDes.length;i++)
		nodesDes[i].display();
}
*/
...
MySceneGraph.prototype.displayScene = function() {
	...
	var currMatrix = root.transformMatrix;
	for(var i=0;i < root.leaves.length;i++){
		this.scene.pushMatrix();
		this.scene.multMatrix(currMatrix);
	...
	for(var j=0;j < root.children.length; j++){
		this.displaySceneAux(root.children[j],currMatrix,currTextureID,currMaterialID);
	}
	*/
}

MySceneGraph.prototype.displaySceneAux = function(currNodeID, currMatrix, currTextureID, currMaterialID) {
	...
	var newMatrix; 
	mat4.multiply(newMatrix, currMatrix, node.transformMatrix);
	...
	for(var j=0;j < root.children,length; j++)
		this.displaySceneAux(newMatrix,newTextureID,newMaterialID);
}